...
output_dir = '.\results_step3_ERP_motionSelf\';
...
% Motion self --> self bounce kinematics
...
for p = [1:40 43:44 47:50 55:56 59:80]          % only participants with OK mocap data
    ...
    for cond=1:4
        ...
        for ii = tr_cond       % For each trial that you wanna analyse
            ...
            %%% Find self-motion salient peaks %%%
            ...
            % Find peaks
            [peaks,locpeaks] = findpeaks(Stim_one_trial_motion_self);
            vel_peaks = zeros(1,length(Stim_one_trial_motion_self));
            vel_peaks(locpeaks) = peaks;

            Stim_one_trial_motion_self_peaks = vel_peaks;
            Stim_one_trial_motion_self_peaks(1:cutOnset)=0;

            % Define onset times of the epochs based on these peaks
            onset_times = find(Stim_one_trial_motion_self_peaks>0);
            ...
            if length(onset_times)>0
                ...
                cfg.trl(:,4) = Stim_one_trial_motion_self_peaks(onset_times);     % store mtion-self peak value
...
% At channel of interest given by TRF results --> C3, C4
cluster_timepoints=[];
...
% Average clusters of 'C3' and 'C4'
for cond=1:4
    cluster_timepoints{cond} = (cluster_timepoints{cond}(1,:)) | (cluster_timepoints{cond}(2,:));
end
...
exportgraphics(gcf,[output_dir '\MOD_amp_GA_C3C4.jpg']);
exportgraphics(gcf,[output_dir '\MOD_amp_GA_C3C4.pdf']);
close;
