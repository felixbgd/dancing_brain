...
output_dir = '.\results_step6_ERP_socialCoord_splitSelf\';
...
% Motion self --> self and other bounce kinematics
...
for p = [1:40 43:44 47:50 55:56 59:80]          % only participants with OK mocap data
    ...
    for cond=1:4
        ...
        for ii = tr_cond       % For each trial that you wanna analyse
            ...
            %%% Find social coordination (IMS - interpersonal movement synchrony) salient peaks %%%
            % Find motion stim
            iSelfParticipant = p; pm = 10;        % bounce            

            % Self
            Stim_one_trial_motion_self_signed = cell2mat(allpred_motion_pm.motion_pms{iSelfParticipant,ii});
            Stim_one_trial_motion_self_signed = Stim_one_trial_motion_self_signed(:,1:lensong);
            Stim_one_trial_motion_self_signed = Stim_one_trial_motion_self_signed(pm,:);

            % Other
            Stim_one_trial_motion_other_signed = cell2mat(allpred_motion_pm.motion_pms{iOtherParticipant,ii}); sz_feat=[1]; feat_names={'Motion Self'};
            Stim_one_trial_motion_other_signed = Stim_one_trial_motion_other_signed(:,1:lensong);
            Stim_one_trial_motion_other_signed = Stim_one_trial_motion_other_signed(pm,:);

            % IMS peaks
            sync = sign(Stim_one_trial_motion_self_signed) .* sign(Stim_one_trial_motion_other_signed);
            sync_peaks = [0 diff(sync)];
            sync_peaks(1:cutOnset)=0;

            Stim_one_trial_IMS_peaks = sync_peaks;

            % Find IMS peaks that are time locked to a change in other 
            sign_other = sign(Stim_one_trial_motion_other_signed);
            sign_other_peaks = [0 diff(sign_other)];
            sign_other_peaks(1:cutOnset)=0;
       
            % And remove these IMS peaks, therefore only focus on IMS peaks
            % time locked to a change in self!
            Stim_one_trial_IMS_peaks(abs(sign_self_peaks)>0)=0;
        
            % Define onset times of the epochs based on these peaks
            onset_times = find(abs(Stim_one_trial_IMS_peaks)>0);
            if length(onset_times)>0
                ...
                cfg.trl(:,4) = Stim_one_trial_IMS_peaks(onset_times);     % store coord peak value
...
% At channel of interest given by TRF results --> Oz
cluster_timepoints=[];
    ...
    if ~isempty(cluster_timepoints{cond})
        ...
        if nb_areas > 0
            ...
            for area=1:nb_areas
                ...
                exportgraphics(gcf,[output_dir '\Oz_' cond_names{cond} '_CLUSTER' num2str(area) '.jpg']);
                exportgraphics(gcf,[output_dir '\Oz_' cond_names{cond} '_CLUSTER' num2str(area) '.pdf']);
                close;
            end
        end
    end
...
exportgraphics(gcf,[output_dir '\MOD_amp_GA_Oz.jpg']);
exportgraphics(gcf,[output_dir '\MOD_amp_GA_Oz.pdf']);
close;
