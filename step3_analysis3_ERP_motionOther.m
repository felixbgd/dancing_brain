...
output_dir = '.\results_step3_ERP_motionOther\';
...
% Motion self --> other bounce kinematics
...
for p = [1:40 43:44 47:50 55:56 59:80]          % only participants with OK mocap data
    ...
    for cond=1:4
        ...
        for ii = tr_cond       % For each trial that you wanna analyse
            ...
            %%% Find other-motion salient peaks %%%
            ...
            % Find peaks
            [peaks,locpeaks] = findpeaks(Stim_one_trial_motion_other);
            vel_peaks = zeros(1,length(Stim_one_trial_motion_other));
            vel_peaks(locpeaks) = peaks;          % <----
             
            Stim_one_trial_motion_other_peaks = vel_peaks;
            Stim_one_trial_motion_other_peaks(1:cutOnset)=0;

            % Define onset times of the epochs based on these peaks
            onset_times = find(abs(Stim_one_trial_motion_other_peaks)>0);
            ...
            if length(onset_times)>0
                ...
                cfg.trl(:,4) = Stim_one_trial_motion_other_peaks(onset_times);     % store motion-other peak value
