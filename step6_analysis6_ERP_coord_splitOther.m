...
%           ERP analysis (Coord - split Other)             %
...
output_dir = '.\results_step6_ERP_socialCoord_splitOther\';
...
for p = [1:40 43:44 47:50 55:56 59:80]          % only participants with OK mocap data
    ...
    for cond=1:4
        ...
        for ii = tr_cond       % For each trial that you wanna analyse
            ...
            % Find IMS peaks that are time locked to a change in self 
            sign_self = sign(Stim_one_trial_motion_self_signed);
            sign_self_peaks = [0 diff(sign_self)]; 
            sign_self_peaks(1:cutOnset)=0; 
       
            % And remove these IMS peaks, therefore only focus on IMS peaks
            % time locked to a change in other!
